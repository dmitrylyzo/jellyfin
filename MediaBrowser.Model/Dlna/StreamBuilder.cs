    ...
    public class StreamBuilder
    {
        ...
        public void CheckCompatibility(MediaType mediaType, MediaSourceInfo mediaSource, DeviceProfile profile)
        {
            ...
            foreach (var stream in mediaSource.MediaStreams)
            {
                ...
                if (!stream.DirectPlayErrors.HasValue)
                {
                    ...
                    stream.DirectPlayErrors = GetCompatibility(mediaType, mediaSource, stream, profile, isSecondaryAudio);
        ...
        private TranscodeReason GetCompatibilityVideo(MediaSourceInfo mediaSource, MediaStream videoStream, DeviceProfile profile)
        {
            ...
            foreach (var directPlayProfile in profile.DirectPlayProfiles)
            {
                ...
                {
                    var currentContainerSupported = directPlayProfile.SupportsContainer(container);

                    if (currentContainerSupported)
                    {
                        containerSupported = true;
                    }

                    // In some profiles, VideoCodec may be empty, assuming that only codecs supported by the container are used.
                    // In such cases, check the codec only if the container is supported.
                    if ((currentContainerSupported || !string.IsNullOrEmpty(directPlayProfile.VideoCodec))
                        && directPlayProfile.SupportsVideoCodec(videoCodec))
                    {
                        videoSupported = true;
                    }
            ...
            failures |= GetCompatibilityVideoCodec(mediaSource, container, videoStream, profile);

            return failures;
        }

        /// <summary>
        /// Gets video codec compatibility errors, if any.
        /// </summary>
        /// <param name="mediaSource">The <see cref="MediaSourceInfo"/>.</param>
        /// <param name="container">The container.</param>
        /// <param name="videoStream">The <see cref="MediaStream"/> of the video stream.</param>
        /// <param name="profile">The <see cref="DeviceProfile"/>.</param>
        /// <returns>Video codec compatibility errors, if any.</returns>
        private TranscodeReason GetCompatibilityVideoCodec(MediaSourceInfo mediaSource, string container, MediaStream videoStream, DeviceProfile profile)
        {
            string videoCodec = videoStream.Codec;
        ...
        private TranscodeReason GetCompatibilityAudio(MediaSourceInfo mediaSource, MediaStream audioStream, DeviceProfile profile, bool isVideo, bool isSecondaryAudio)
        {
            ...
            foreach (var directPlayProfile in profile.DirectPlayProfiles)
            {
                if (directPlayProfile.Type == profileType)
                {
                    ...
                    // In some profiles, AudioCodec may be empty, assuming that only codecs supported by the container are used.
                    // In such cases, check the codec only if the container is supported.
                    if ((currentContainerSupported || !string.IsNullOrEmpty(directPlayProfile.AudioCodec))
                        && directPlayProfile.SupportsAudioCodec(audioCodec))
                    {
                        audioSupported = true;
                    }
            ...
            failures |= GetCompatibilityAudioCodec(mediaSource, container, audioStream, profile, isVideo, isSecondaryAudio);

            return failures;
        }

        /// <summary>
        /// Gets audio codec compatibility errors, if any.
        /// </summary>
        /// <param name="mediaSource">The <see cref="MediaSourceInfo"/>.</param>
        /// <param name="container">The container.</param>
        /// <param name="audioStream">The <see cref="MediaStream"/> of the audio stream.</param>
        ...
        /// <param name="isVideo">True if source is video.</param>
        /// <param name="isSecondaryAudio">True if secondary audio stream.</param>
        /// <returns>Audio codec compatibility errors, if any.</returns>
        private TranscodeReason GetCompatibilityAudioCodec(MediaSourceInfo mediaSource, string container, MediaStream audioStream, DeviceProfile profile, bool isVideo, bool isSecondaryAudio)
        {
            string audioCodec = audioStream.Codec;
            ...
            StreamInfo playlistItem)
        {
            ...
            var analyzedProfiles = transcodingProfiles
                .Select(transcodingProfile =>
                {
                    ...
                    if (videoStream is not null && options.AllowVideoStreamCopy && ContainerHelper.ContainsContainer(transcodingProfile.VideoCodec, videoCodec))
                    {
                        var failures = GetCompatibilityVideoCodec(playlistItem.MediaSource!, container, videoStream, options.Profile);
                        rank.Video = failures == 0 ? 1 : 2;
                    }

                    if (audioStream is not null && options.AllowAudioStreamCopy && ContainerHelper.ContainsContainer(transcodingProfile.AudioCodec, audioCodec))
                    {
                        var failures = GetCompatibilityAudioCodec(playlistItem.MediaSource!, container, audioStream, options.Profile, true, false);
